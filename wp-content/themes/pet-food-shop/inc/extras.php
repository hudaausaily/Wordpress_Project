 ...
 * @return array
 */
function petfoodshop_body_classes( $classes ) {
...
add_filter( 'body_class', 'petfoodshop_body_classes' );
...
if (!function_exists('petfoodshop_str_replace_assoc')) {

    /**
     * petfoodshop_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function petfoodshop_str_replace_assoc(array $replace, $subject) {
 ...
 */
function petfoodshop_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','pet-food-shop'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'petfoodshop_add_to_cart_fragment' );
...
if ( ! function_exists( 'petfoodshop_breadcrumb_title' ) ) {
	function petfoodshop_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'pet-food-shop' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'pet-food-shop' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'pet-food-shop' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'pet-food-shop' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'pet-food-shop' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'pet-food-shop' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'pet-food-shop' ), (get_the_author( '', false ) ));		
...
function petfoodshop_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','pet-food-shop'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','pet-food-shop'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','pet-food-shop'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','pet-food-shop').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','pet-food-shop').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','pet-food-shop').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','pet-food-shop').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','pet-food-shop'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','pet-food-shop') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_petfoodshop_dismissed_notice_handler', 'petfoodshop_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function petfoodshop_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'petfoodshop_admin_install_plugin' );

function petfoodshop_admin_install_plugin() {
    /**
     * Install Plugin.
     */
